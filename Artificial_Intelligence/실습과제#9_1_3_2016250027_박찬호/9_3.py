...
print("\nc. Now comes the magic. For each test set instance, \n"
      "generate the predictions of the1,000 Decision Trees, \n"
      "and keep only the most frequent prediction \n"
      "(you can use SciPy's mode() function for this). \n"
...
grid_search_cv = GridSearchCV(DecisionTreeClassifier(random_state=42), params, verbose=1, cv=3)
grid_search_cv.fit(X_mini_train, y_mini_train)
...
print("\nd. Evaluate these predictions on the test set: \n"
      "you should obtain a slightly higher accuracy than \n"
      "your first model (about 0.5 to 1.5% higher). \n"
